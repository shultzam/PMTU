...
local function shouldSendPokedexCapture(pokemon_name, isShiny)
    ...
    -- Session-level throttle to avoid burst duplicates from refresh.
    local now = os.time()
    ...
    -- Persisted guard: skip if we've already recorded the same or better shiny state.
...
-- Called by the handler after a successful record to persist state.
...
function onCaptureResolved(pokemon_name, isShiny)
    -- Check if a player is occupying this rack.
    if Player[playerColour] == nil then
        return
    end
